#import "RNDocumentPickerZowie.h"
...
#import "RNCPromiseWrapperZowie.h"
...
@interface RNDocumentPickerZowie () <UIDocumentPickerDelegate, UIAdaptivePresentationControllerDelegate>
@end

@implementation RNDocumentPickerZowie {
    UIDocumentPickerMode mode;
    NSString *copyDestination;
    RNCPromiseWrapperZowie* promiseWrapper;
...
{
    if ((self = [super init])) {
        promiseWrapper = [RNCPromiseWrapperZowie new];
...
- (NSMutableDictionary *)getMetadataForUrl:(NSURL *)url error:(NSError **)error
{
    ...
    [coordinator coordinateReadingItemAtURL:url options:NSFileCoordinatorReadingResolvesSymbolicLink error:&fileError byAccessor:^(NSURL *newURL) {
        ...
        NSError *copyError;
        NSURL *maybeFileCopyPath = copyDestination ? [RNDocumentPickerZowie copyToUniqueDestinationFrom:newURL usingDestinationPreset:copyDestination error:&copyError] : nil;
        ...
        if(!attributesError) {
            ...
            NSLog(@"RNDocumentPickerZowie: %@", attributesError);
        }
...
- (void)pickDirectory:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    reject(@"RNDocumentPickerZowie:pickDirectory", @"pickDirectory is not supported on iOS", nil);
}
