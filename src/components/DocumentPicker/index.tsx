...
export async function pickDirectory<OS extends SupportedPlatforms>(
  ...
  if (Platform.OS === 'ios') {
    ...
    return { uri: result[0]?.uri ? result[0].uri : null };
  } else {
    return NativeDocumentPicker.pickDirectory();
  }
}

export async function pickSingle<OS extends SupportedPlatforms>(
